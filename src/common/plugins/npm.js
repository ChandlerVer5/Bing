...
export default (dir) => {
  ...
  const setConfig = (config) => fs.writeFileSync(packageJson, JSON.stringify(config, null, 2))
    ...
    install(name, options = {}) {
      ...
      return fetch(`${API_BASE}${name}`)
        ...
        .then((json) => {
          ...
          return installPackage(json.versions[versionToInstall].dist.tarball, path.join(dir, 'node_modules', name), middleware)
        })
        .then(() => {
    ...
    uninstall(name) {
      ...
      return removeDir(modulePath)
        .then(() => {
          ...
          json.dependencies = Object.keys(json.dependencies || {}).reduce((acc, key) => {
            if (key !== name) {
              return {
                ...acc,
