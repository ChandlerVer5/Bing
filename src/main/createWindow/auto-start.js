...
const isLinux = ['win32', 'darwin'].includes(process.platform) === -1

if (isLinux) {
  appLauncher = new AutoLaunch({ name: 'Cerebro' })
}

const isEnabled = () => (isLinux ? appLauncher.isEnabled() : Promise.resolve(app.getLoginItemSettings().openAtLogin))
