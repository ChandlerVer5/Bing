...
export default class Settings extends Component {
  ...
  renderSetting(key) {
    ...
    return <FormItem key={key} label={label || key} value={value} onChange={(newValue) => this.changeSetting(this.props.name, key, newValue)} {...restProps} />
  }

  render() {
    return <div className={styles.settingsWrapper}>{Object.keys(this.props.settings).map(this.renderSetting)}</div>
  }
}
