  ...
  renderDescription(repo) {
    return (
      <Preload promise={getReadme(repo)}>
        {/* {(content) => <ReactMarkdown source={content} className={styles.markdown} transformImageUri={(src) => urlTransform(repo, src)} />} */}
      </Preload>
    )
  }

  render() {
    const { name, version, description, repo, isInstalled, isDebugging, installedVersion, isUpdateAvailable } = this.props
      ...
      <div className={styles.preview} key={name}>
        ...
        <KeyboardNav>
          <div className={styles.header}>
            {settings && <KeyboardNavItem onSelect={() => this.setState({ showSettings: !this.state.showSettings })}>Settings</KeyboardNavItem>}
