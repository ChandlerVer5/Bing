...
import * as statusBar from '@/renderer/actions/statusBar'
...
const updatePlugin = (update, name) => {
  loadPlugins().then((plugins) => {
    ...
    update(name, {
      ...
      getPreview: () => <Preview {...updatedPlugin} key={Math.random()} onComplete={() => updatePlugin(update, name)} />
    })
  })
}
...
const categorize = (plugins, callback) => {
  ...
  categories.forEach((category) => {
    ...
    if (matched.length < 0) result.push(title, ...matched)
    remainder = others
  })
